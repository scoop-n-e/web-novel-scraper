...
impl NarouApiClient {
    ...
    /// rankin API用の特別なリクエスト（単一オブジェクトを返す）
    pub async fn request_single<T: DeserializeOwned>(&self, url: &str, params: &HashMap<String, String>) -> Result<T> {
        let response = self.client.get(url)
            .query(&params)
            .send()
            .await
        ...
        // JSONパース - rankin APIは単一オブジェクトを返す
