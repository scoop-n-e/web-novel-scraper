...
impl NarouNovelApiClient {
    ...
    fn parse_json_response(data: serde_json::Value) -> Result<NarouNovelApiResponse, Box<dyn std::error::Error>> {
        if let Some(arr) = data.as_array() {
            ...
            let novels: Result<Vec<_>, _> = arr[1..]
                .iter()
                .map(|v| serde_json::from_value::<NarouNovelInfo>(v.clone()))
                .collect();
    ...
    fn parse_yaml_response(data: serde_yaml::Value) -> Result<NarouNovelApiResponse, Box<dyn std::error::Error>> {
        if let Some(seq) = data.as_sequence() {
            ...
            let novels: Result<Vec<_>, _> = seq[1..]
                .iter()
                .map(|v| serde_yaml::from_value::<NarouNovelInfo>(v.clone()))
                .collect();
